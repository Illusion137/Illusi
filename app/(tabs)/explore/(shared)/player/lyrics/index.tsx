...
function compute_ssm(frames: number[][]): number[][] {
	...
	return Array.from({ length: F }, (_, i) =>
		Array.from({ length: F }, (__, j) => cosine_sim(frames[i], frames[j]))
	);
}
...
function compute_novelty(ssm: number[][], ks: number): number[] {
	...
	return Array.from({ length: F }, (_, k) => {
		...
		const mean = (pairs: [number, number][]) => {
			let s = 0; pairs.forEach(([i, j]) => { s += ssm[i][j]; }); return s / pairs.length;
		};
...
function normalize_section_name(h: string | null): string {
	...
	return h.toLowerCase().replace(/\s*\d+\s*$/, "").replace(/[^a-z\-]/g, "").trim();
}

function find_best_in_range(arr: number[], from: number, to: number): number {
	let best = -Infinity, idx = Math.round((from + to) / 2);
...
function sanitize_section_times(times: number[], syl_weights: number[], duration: number): number[] {
	...
	if (total_min >= duration) {
		let t = 0;
		return [0, ...syl_weights.slice(0, -1).map((w) => { t += (w / total_syl) * duration; return t; })];
	}

	// Clamp raw boundaries, sort interior ones (second pass may have reordered them)
	const out = [0, ...times.slice(1).map((t) => Math.max(0, Math.min(t, duration - 1))).sort((a, b) => a - b)];
...
function count_syllables(text: string): number {
	return text.split(/\s+/).filter(Boolean).reduce((sum, w) => sum + count_syllables_word(w), 0);
}
...
export default function AudioPlayerLyrics() {
	...
	async function analyze_waveform(parsed_sections: LyricsSection[], t: Track) {
		...
		try {
			...
			const frames: number[][] = Array.from({ length: F }, (_, fi) =>
				smoothed.slice(fi * frame_size, fi * frame_size + frame_size)
			);

			// Self-Similarity Matrix: sim[i][j] = cosine similarity of frame i and j.
			// High off-diagonal similarity = repeated sections (e.g. two choruses).
			const ssm = compute_ssm(frames);
			...
			const fallback = sanitize_section_times(
				[0, ...sw.slice(0, -1).map(((t => (w: number) => { t += (w / total) * duration; return t; })(0)))],
				sw,
				duration
			);
