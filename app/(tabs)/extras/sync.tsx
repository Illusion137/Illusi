...
export default function ExtraSyncScreen() {
	...
	useEffect(() => {
		supabase.auth.getSession().then(({ data: { session } }) => set_session(session));
	...
	const last_synced_str = last_synced.getTime() === 0
		? 'Never'
			...
			<ScrollView style={styles.container}>
				<View style={styles.section}>
					...
					<View style={styles.info_row}>
						...
						<Text style={styles.value} numberOfLines={1}>{session.user.email}</Text>
					</View>
					<View style={styles.info_row}>
				...
				<TouchableHighlight
					activeOpacity={0.6}
					underlayColor={colors.highlightPressColor}
				...
				<View style={styles.section}>
					...
					<View style={styles.uuid_container}>
						{Prefs.get_pref('user_uuid').split('-').map((part, i) => (
							<Text key={i} style={styles.uuid_text}>{part}</Text>
						))}
					</View>
				</View>
		...
		<ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
			<View style={styles.section}>
				...
				<Text style={styles.description}>
					{is_signup
						? 'Create an account to back up your library and sync across devices.'
						: 'Sign in to restore your library backup or sync across devices.'
					}
				</Text>
				...
				<TouchableHighlight
					...
					disabled={loading || !email || !passkey}
					style={[styles.submit_btn, (!email || !passkey) && styles.btn_disabled]}>
					<View style={styles.btn_content}>
						{loading
							? <ActivityIndicator color="#fff" />
							: <Text style={styles.submit_text}>{is_signup ? 'Create Account' : 'Sign In'}</Text>
						}
					</View>
				</TouchableHighlight>
				...
				<TouchableHighlight
					...
					onPress={() => { set_is_signup(!is_signup); set_error(null); }}>
