...
export default function AudioPlayerSettings() {
	...
	const RateChipRenderer = (props: { entry: [keyof typeof rate_presets, (typeof rate_presets)[keyof typeof rate_presets]] }) => (
		<TouchableOpacity style={styles.rate_chip} onPress={async () => update_rate(rate_presets[props.entry[0]])}>
		...
		<>
			...
			<View style={{ flex: 1, backgroundColor: colors.background }}>
				...
				<View style={styles.slider}>
					<Slider value={rate} onValueChange={async (value) => update_rate(value[0])} thumbTintColor={colors.primary} thumbStyle={{ width: 15, height: 15 }} thumbTouchSize={{ width: 1, height: 1 }} minimumTrackTintColor={colors.primary} maximumTrackTintColor="#DADADA40" step={0.05} debugTouchArea={false} minimumValue={min_rate} maximumValue={max_rate} />
				</View>
				...
				<View style={{ width: "100%", flexDirection: "row" }}>
					<View style={styles.slider_small}>
						<Slider value={crossfade_seconds} onValueChange={async (value) => update_crossfade_seconds(value[0])} thumbTintColor={colors.primary} thumbStyle={{ width: 15, height: 15 }} thumbTouchSize={{ width: 1, height: 1 }} minimumTrackTintColor={colors.primary} maximumTrackTintColor="#DADADA40" step={0.1} debugTouchArea={false} minimumValue={min_crossfade} maximumValue={max_crossfade} />
					</View>
