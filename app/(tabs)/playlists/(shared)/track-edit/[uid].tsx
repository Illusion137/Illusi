...
function EditArtistPreview(props: { track: Track; artist: NamedUUID; index: number; set_artists_state: SetArtistsState }) {
	...
	return (
		<View style={{ margin: 20, height: 200 }}>
			...
			<TextInput defaultValue={props.artist.name} autoCorrect={false} placeholder="Enter Artist Name" placeholderTextColor={colors.searchPlaceholder} style={styles.search_input} onFocus={() => set_input_focused(true)} onBlur={() => set_input_focused(false)} onChangeText={on_name_change} onEndEditing={on_name_submit} onSubmitEditing={on_name_submit} />
...
export default function EditTrackModal() {
	...
	const [close_albums, set_close_album] = useState<NamedUUID[]>([]);
	...
	function on_album_name_change(new_album_name: string) {
		...
		set_close_album(all_albums_ref.current?.filter((album) => album.name.toLowerCase().includes(album_ref.current?.name.toLowerCase() ?? "")));
	}
		...
		<View style={{ flex: 1, backgroundColor: colors.background }}>
			...
			<ScrollView scrollToOverflowEnabled={false}>
				...
				<View style={{ flexDirection: "row", alignItems: "flex-end" }}>
					...
					{editing_title_state === "LOADING" ? <ActivityIndicator size={30} /> : editing_title_state === "COMPLETE" ? <Ionicons name="checkmark" size={30} color={colors.green} /> : null}
				</View>
				<View style={{ marginLeft: 30, height: 0.5, backgroundColor: colors.text, width: "85%" }} />
				...
				<View style={{ flexDirection: "row", marginTop: 10 }}>
					...
					<View style={{ width: "75%", right: 10, marginTop: 10 }}>
						<View style={{ flexDirection: "row", alignItems: "flex-end" }}>
							<TextInput defaultValue={track_ref.current?.album?.name} autoCorrect={false} placeholder="Enter Album Name" placeholderTextColor={colors.searchPlaceholder} style={styles.search_input} onChangeText={on_album_name_change} onEndEditing={on_album_name_submit} onSubmitEditing={on_album_name_submit} onBlur={() => set_album_name_input_focused(false)} onFocus={() => set_album_name_input_focused(true)} />
							{editing_album_name_state === "LOADING" ? <ActivityIndicator size={30} /> : editing_album_name_state === "COMPLETE" ? <Ionicons name="checkmark" size={30} color={colors.green} /> : null}
						</View>
				...
				{album_name_input_focused ? (
					<ScrollView style={{ width: "103%", maxHeight: 400, backgroundColor: "#00000070", top: -20, zIndex: 5, paddingHorizontal: 10 }}>
						{close_albums.map((album) => (
							<View key={album.name + (album.uri ?? "")} style={{ flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 1 }}>
								<Text style={{ color: colors.text }}>{album.name}</Text>
								<Text style={{ color: colors.text }}>{album.name}</Text>
							</View>
						))}
					</ScrollView>
