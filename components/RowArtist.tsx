...
export default function RowArtist(props: {
    ...
    const service_uri = props.artist_data.name.uri ? 
        music_service_uri_to_music_service(split_uri(props.artist_data.name.uri)[0]) 
        : undefined;
    ...
    return (
        <TouchableOpacity onPress={on_press} style={{padding: 10, paddingHorizontal: size * 0.08, alignItems: 'center'}}>
            <View>
                ...
                {service_uri ? 
                    <Fontisto name={service_icon_map[service_uri].name}
                        size={size * 0.18}
