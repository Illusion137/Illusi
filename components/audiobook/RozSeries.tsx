	...
	return (
		<ContextMenuView
			...
			onPressMenuItem={async ({ nativeEvent }) => {
				await ContextResolver.resolve_series_context(props.series_name, props.novels, nativeEvent.actionKey as ContextResolver.SeriesContextKeys, { on_open: props.on_press, on_resume: props.on_press, on_refresh: props.on_refresh });
			}}>
			<View style={{ ...props.container_style, width: props.size }}>
				<TouchableOpacity
					activeOpacity={cover_touch_active_opacity}
					onPress={() => props.on_press?.(props.series_name, props.novels)}
					onLongPress={() => props.on_long_press?.(props.series_name, props.novels)}
...
const theme_styles = (colors: Prefs.Theme["colors"]) =>
	StyleSheet.create({
		cover: { position: "absolute", borderRadius: 2, borderWidth: 1, borderColor: colors.line, borderTopWidth: 0.4, borderRightWidth: 0.6, backgroundColor: colors.shelf },
