...
export default function PlaylistBase(props: PlaylistProps){
    ...
    function getShortcut(): ShortcutOptions{
        const key = props.type === "UUID" ? props.uuid : 
            props.type === "DEFAULT_PLAYLIST" ? props.default_playlist_title : "";
        return {
    ...
    const write_playlist_uuid = props.type === "URI" 
        ? Constants.library_write_playlist 
            : props.type === "WRITE_PLAYLIST"
        ? props.write_playlist_uuid : undefined;

	const render_track = (item: {item: Track}) => (
		<TrackComponent playlist_uuid={playlist_data?.uuid}
		...
		<View style={styles.playlist_list_header}>
            ...
            <View style={{flexDirection: 'row', top: 10, width: '100%'}}>
                <View style={styles.playlist_buttons_container}>
                    {props.type !== "WRITE_PLAYLIST" ? <IoniconsTouchableOpacity on_press={() => { play_order(tracks); }} 
