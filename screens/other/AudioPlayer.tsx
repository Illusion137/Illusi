...
function AudioPlayer(props: {
    ...
    const panel_ref = useRef<SlidingUpPanel>(null) as RefObject<SlidingUpPanel>;
    ...
    async function remove_track_from_queue(item: IllusiveType.Track){
        const current_track_index = await TrackPlayer.getActiveTrackIndex();
        if(current_track_index === undefined) return;
    ...
    const render_now_playing_item = (item: { item: IllusiveType.Track }) => <TrackComponentBase track_data={item.item} on_press={() => {}} on_long_press={() => {}} disabled={true} />;
                    ...
                    <View style={{ bottom: 35 }}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 35, top: 10 }}>
                            ...
                            <TouchableOpacity disabled={lyrics_loading_state === "FAILED"} onPress={async () => {
                                ...
                                const read_lyrics = await SQLTracks.read_track_lyrics(track);
                                if(read_lyrics && typeof read_lyrics !== 'object') {
                                ...
                                SQLTracks.save_track_lyrics(track, lyrics);
                                set_lyrics_loading_state("DOWNLOADED");
                        ...
                        <SwipeListView
                            ...
                            ListHeaderComponent={() =>
                                <View style={{ flex: 1, width: '100%', height: 140 }}>
                                    ...
                                    {render_now_playing_item({item: now_playing_state.queue_data[0]})}
