...
import { clean_youtube_title } from "@illusive/parsers/youtube_parser";
import TrackHorizontalScrolls from "@components/TrackHorizontalScrolls";
import usePTheme from "@hooks/usePTheme";

type MusiExploreResult = ReturnType<typeof musi_parse_explore>;
let musi_explore_data: MusiExploreResult;
export default function MusiExplore() {
	...
	const [musi_explore, set_musi_explore] = useState<MusiExploreResult>();
	...
	useEffect(() => {
		(async function () {
			...
			musi_explore_data.top_tracks = SQLTracks.add_playback_saved_data_to_tracks(musi_explore_data.top_tracks);
			set_musi_explore(musi_explore_data);
		})();
	}, []);
		...
		<View style={{ flexDirection: "column" }}>
			<TouchableOpacity
				...
				onPress={() => {
					...
					const uri = placement.item.playlist.youtube_playlist_id !== undefined ? create_uri("youtube", placement.item.playlist.youtube_playlist_id) : create_uri("musi", placement.item.playlist.musi_playlist_id!);
					const compact_playlist: CompactPlaylist = { title: { name: placement.item.playlist.name, uri: null }, artist: [{ name: "Musi", uri: null }], artwork_url: placement.item.icon_image_url };
		...
		<ScrollView>
			{musi_explore === undefined ? (
				...
				<ImageBackground imageStyle={{ opacity: 0.08 }} style={{ height: 300, justifyContent: "flex-end" }} source={{ uri: musi_explore.modules[0].placements[0].icon_image_url }}>
					...
					<Text numberOfLines={1} style={{ color: colors.subtext, fontSize: 18, fontWeight: "300", marginHorizontal: 15 }}>
						{musi_explore.modules[0].placements[0].track?.user}
					</Text>
					<Text numberOfLines={1} style={{ color: colors.text, fontSize: 30, fontWeight: "bold", marginLeft: 10, marginRight: 50 }}>
						{clean_youtube_title(musi_explore.modules[0].placements[0].track?.title ?? "")}
					</Text>
				</ImageBackground>
			)}
			<FlatList style={{ flex: 1 }} horizontal={true} data={musi_explore === undefined ? [] : musi_explore.modules.find((module) => module.type === "scroller" && module.placements.length > 0)?.placements} renderItem={render_top_playlist} />
			<View style={{ height: 22 }} />
			{musi_explore !== undefined
				? musi_explore.modules
						.filter((module) => module.type === "scroller" && module.placements.length > 0)
						.slice(1)
						.map((module, index) => (
							<ExtrasSectionButton
			...
			{musi_explore !== undefined ? (
				<>
					...
					<View style={styles.line_long} />
					<TrackHorizontalScrolls title="Top Tracks" tracks={musi_explore.top_tracks.slice(0, 20)} height={5} />
				</>
			) : null}
...
const theme_styles = (colors: Prefs.Theme["colors"]) =>
	StyleSheet.create({
		topContainer: { flex: 1, backgroundColor: colors.background },
